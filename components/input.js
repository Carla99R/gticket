...
export default class Input extends Base {
    ...
    async componentDidMount() {
        ...
        if (this.props.type == "numero" || this.props.type == "decimal" || this.props.type == "passwordnum" || this.props.type == "telfmovistar" || this.props.type == "telfdigitel" || this.props.type == "phone") {
    ...
    validCatacterForm(e) {
        ...
        if (String(valor).length <= this.state.limite) {
            ...
            if (this.state.type == "numero" || this.state.type == "phone") {
                var vv = String(valor).replace(/[^0-9]+/gi, '')
    ...
    clear() {
        this.setState({
            valor: '',
            error: '',
            limite: this.props.limite ? this.props.limite : 11,
            ayuda: this.props.ayuda,
            ico: this.props.ico
        });
    ...
    valid(callback = function () {
        ...
        if (this.state.valor && this.state.valor != undefined && String(this.state.valor).length >= this.state.min) {
            if (this.state.type == "email") {
                if (!this.validMail(String(this.state.valor))) {
                    this.setState({
                        error: "Debe insertar un correo válido",
                        viewError: this.props.viewError ?
                            this.props.viewError : true
                    })
                    this.props.handleE("Debe insertar un correo válido")
                    return false
                ...
                const identificador = String(this.state.valor.substring(0, 4))
                switch (identificador) {
                    ...
                    case "0416":
                    case "0426":
                        if (!this.onlyPhone(String(this.state.valor), [identificador])) {
                            console.log("RESP")
                            console.log(this.onlyPhone(String(this.state.valor), [identificador]))
                            this.setState({
                                error: "Debe insertar un teléfono válido",
                                viewError: this.props.viewError ?
                                    this.props.viewError : true
                            })
                            this.props.handleE("Debe insertar un teléfono válido")
                            return false
                        } else {
                            this.setState({error: ""})
                            this.props.handleE("")
            ...
            if (this.state.min == 0 || String(this.state.valor).length == 0) {
                ...
                callback("Ingrese campo " + this.state.placeholder)
                this.props.handleE("Obligatorio")
                // this.refs.mInput.focus()
                ...
                if (this.state.min == this.state.limite) {
                    this.setState({
                        error: "Debe ser igual a " + String(this.state.min) + " " + tt + " ",
                        viewError: this.props.viewError ? this.props.viewError : true
                    })
                    callback("Debe ser igual a " + String(this.state.min) + " " + tt + " ")
                    this.props.handleE("Debe ser igual a " + String(this.state.min) + " " + tt + " ")

                    // this.refs.mInput.focus()
                } else {

                    this.setState({
                        error: "Debe ser mayor o igual a " + String(this.state.min) + " " + tt + " ",
                        viewError: this.props.viewError ? this.props.viewError : true
                    })
                    callback("Debe ser mayor o igual a " + String(this.state.min) + " " + tt + " ")
                    this.props.handleE("Debe ser mayor o igual a " + String(this.state.min) + " " + tt + " ")
        ...
        return (<>
                <View style={[{paddingTop: 4}]}>
                    <TextField
                        ...
                        style={[styles.input, {
                            borderColor: this.state.foco ? "#03A9F4" : "#fff",
                            backgroundColor: "#fff"
                        }]}
                        onChangeText={(e) => {
                            this.validCatacterForm(e)
                        }}
                        transformer={(e) => {
                            if (this.state.type == "password" || this.state.type == "passwordnum") {
                        ...
                        onBlur={() => {
                            this.valid()
                            this.props.changeStatus(this.state.valor)
